...
// the biggest difference is that a rune is unsigned instead of signed (because Go guarantees what a right shift on a
// signed number will do, whereas C does not) it is also an imitation so we can license it under looser terms than the
// Go source
#define badrune 0xFFFD

// encoded must be at most 4 bytes
size_t
uiprivUTF8EncodeRune (uint32_t rune, char *encoded)
{
  uint8_t b;
  uint8_t c = 0;
  uint8_t d = 0;
...
const char *
uiprivUTF8DecodeRune (const char *s, size_t nElem, uint32_t *rune)
{
  uint8_t c;
  ...
  uint8_t lowestAllowed  = 0x80;
  uint8_t highestAllowed = 0xBF;
  switch (b)
    {
    case 0xE0:
...
size_t
uiprivUTF16EncodeRune (uint32_t rune, uint16_t *encoded)
{
  ...
  rune -= 0x10000;
  const uint16_t low = rune & 0x3FF;
  rune >>= 10;
  const uint16_t high = rune & 0x3FF;
...
const uint16_t *
uiprivUTF16DecodeRune (const uint16_t *s, const size_t nElem, uint32_t *rune)
{
...
size_t
uiprivUTF8RuneCount (const char *s, size_t nElem)
{
  size_t   len;
  uint32_t rune;
  ...
  if (nElem != 0)
    {
      ...
      while (nElem != 0)
        {
          const char *u = uiprivUTF8DecodeRune (t, nElem, &rune);
          len++;
          nElem -= u - t;
          t = u;
        }
...
size_t
uiprivUTF8UTF16Count (const char *s, size_t nElem)
{
  size_t   len;
  uint32_t rune;
  uint16_t encoded[2];
...
size_t
uiprivUTF16RuneCount (const uint16_t *s, size_t nElem)
{
  size_t   len;
  uint32_t rune;
  ...
  if (nElem != 0)
    {
      ...
      while (nElem != 0)
        {
          const uint16_t *u = uiprivUTF16DecodeRune (t, nElem, &rune);
          len++;
          nElem -= u - t;
          t = u;
        }
...
size_t
uiprivUTF16UTF8Count (const uint16_t *s, size_t nElem)
{
  size_t   len;
  uint32_t rune;
  char     encoded[4];
