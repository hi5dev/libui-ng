...
static BOOL
onWM_COMMAND (uiControl *c, HWND hwnd, WORD code, LRESULT *lResult)
{
...
uiNewButton (const char *text)
{
  ...
  wtext   = toUTF16 (text);
  b->hwnd = uiWindowsEnsureCreateControlHWND (0, L"button", wtext, BS_PUSHBUTTON | WS_TABSTOP, hInstance, NULL, TRUE);
  uiprivFree (wtext);
