...
static uiTableValue *
modelCellValue (uiTableModelHandler *, uiTableModel *, const int row, const int col)
{
  char buf[256];
...
static void
modelSetCellValue (uiTableModelHandler *, uiTableModel *m, const int row, const int col, const uiTableValue *val)
{
  if (row == 9 && col == 2)
  ...
  if (col == 6)
    {
      const int prevYellowRow = yellowRow;
      yellowRow               = row;
...
void
headerVisibleToggled (uiCheckbox *c, void *data)
{
  uiTable *t = data;
...
void
selectionModeOnSelected (uiCombobox *c, void *data)
{
  uiTable  *t = data;
  const int i = uiComboboxSelected (c);
...
static void
changedColumnID (uiSpinbox *, void *data)
{
  uiTable *t = data;
...
static void
changedColumnWidth (uiSpinbox *, void *data)
{
  uiTable *t = data;
...
static void
onRowClicked (uiTable *, const int row, void *)
{
  char str[128];
...
static void
onRowDoubleClicked (uiTable *, const int row, void *data)
{
  char str[128];
...
static void
headerOnClicked (uiTable *t, const int col, void *data)
{
  static int prev = 0;
...
static void
onSelectionChanged (uiTable *t, void *)
{
  int               sum = 0;
  char              str[128];
...
static void
selectChecked (uiButton *, void *data)
{
  int      rows[sizeof (checkStates) / sizeof (*checkStates)];
  int      numChecked = 0;
  uiTable *t          = data;

  for (unsigned i = 0; i < sizeof (checkStates) / sizeof (*checkStates); ++i)
    if (checkStates[i])
      rows[numChecked++] = i;
...
makePage16 (void)
{
  ...
  uiCheckbox *headerVisible = uiNewCheckbox ("Header Visible");
  ...
  uiCombobox *selectionMode = uiNewCombobox ();
