...
#define SETTOO(what, name, n)                                                                                         \
  static void set##what##Too##name (uiButton *this, void *data) { ui##what##SetValue (ui##what (data), n); }
...
updateCB ()
{
  ...
  (void)sprintf (str, "%d", uiComboboxNumItems (cbox));

  uiLabelSetText (cboxLbl, str);
}

static void
appendCBRB (uiButton *, void *)
{
...
static void
onCBChanged (uiCombobox *c, void *data)
{
  printf ("%s combobox changed to %d\n", (char *)data, uiComboboxSelected (c));
  uiEditableComboboxSetText (editable, "changed");
}

static void
onECBChanged (uiEditableCombobox *c, void *data)
{
  char *t = uiEditableComboboxText (c);
  printf ("%s combobox changed to %s\n", (char *)data, t);
  uiFreeText (t);
}

static void
onRBSelected (uiRadioButtons *r, void *)
{
  printf ("radio buttons %d\n", uiRadioButtonsSelected (r));
}

static void
selectSecond (uiButton *, void *)
{
  uiRadioButtonsSetSelected (rb, 1);
}

static void
selectNone (uiButton *, void *)
{
  uiRadioButtonsSetSelected (rb, -1);
}

static void
sliderEnableToolTip (uiButton *, void *data)
{
...
static void
sliderDisableToolTip (uiButton *b, void *data)
{
...
static void
setSliderRange (uiSpinbox *spinbox, void *data)
{
  uiSlider *s = data;
...
static void
onRangeSliderChanged (uiSlider *s, void *data)
{
  char     str[128];
  uiLabel *lbl = data;

  (void)sprintf (str, "%d", uiSliderValue (s));
  uiLabelSetText (lbl, str);
}

static void
onSliderReleased (uiSlider *s, void *)
{
  printf ("on Slider released\n");
  const int value = uiSliderValue (s);
...
makePage4 (void)
{
  ...
  uiButton *b = uiNewButton ("Enable ToolTip");
  ...
  uiSpinbox *xsb = uiNewSpinbox (-40, 40);
  ...
  hbox          = newHorizontalBox ();
  uiSlider *xsl = uiNewSlider (-40, 40);
