...
makeBasicControlsPage (void)
{
  ...
  uiBoxAppend (hbox, uiControl (uiNewButton ("Button")), 0);
  uiBoxAppend (hbox, uiControl (uiNewCheckbox ("Checkbox")), 0);

  uiBoxAppend (vbox, uiControl (uiNewLabel ("This is a label.\nLabels can span multiple lines.")), 0);

  uiBoxAppend (vbox, uiControl (uiNewHorizontalSeparator ()), 0);

  uiGroup *group = uiNewGroup ("Entries");
  ...
  uiFormAppend (entryForm, "Entry", uiControl (uiNewEntry ()), 0);
  uiFormAppend (entryForm, "Password Entry", uiControl (uiNewPasswordEntry ()), 0);
  uiFormAppend (entryForm, "Search Entry", uiControl (uiNewSearchEntry ()), 0);
  uiFormAppend (entryForm, "Multiline Entry", uiControl (uiNewMultilineEntry ()), 1);
  uiFormAppend (entryForm, "Multiline Entry No Wrap", uiControl (uiNewNonWrappingMultilineEntry ()), 1);
...
static void
onSpinboxChanged (uiSpinbox *s, void *)
{
...
static void
onSliderChanged (uiSlider *s, void *)
{
...
makeNumbersPage ()
{
  ...
  uiGroup *group = uiNewGroup ("Numbers");
  uiGroupSetMargined (group, 1);
  ...
  uiProgressBar *ip = uiNewProgressBar ();
  ...
  uiEditableCombobox *ecbox = uiNewEditableCombobox ();
...
static void
onOpenFileClicked (uiButton *b, void *data)
{
...
static void
onOpenFolderClicked (uiButton *b, void *data)
{
  uiEntry *entry = uiEntry (data);

  char *filename = uiOpenFolder (mainwin);
  if (filename == NULL)
    {
      uiEntrySetText (entry, "(cancelled)");
      return;
...
static void
onSaveFileClicked (uiButton *b, void *data)
{
  uiEntry *entry = uiEntry (data);

  char *filename = uiSaveFile (mainwin);
  if (filename == NULL)
    {
      uiEntrySetText (entry, "(cancelled)");
      return;
...
makeDataChoosersPage (void)
{
  ...
  uiBoxAppend (vbox, uiControl (uiNewDatePicker ()), 0);
  uiBoxAppend (vbox, uiControl (uiNewTimePicker ()), 0);
  uiBoxAppend (vbox, uiControl (uiNewDateTimePicker ()), 0);

  uiBoxAppend (vbox, uiControl (uiNewFontButton ()), 0);
  uiBoxAppend (vbox, uiControl (uiNewColorButton ()), 0);
  ...
  uiButton *button = uiNewButton ("  Open File  ");
  uiEntry  *entry  = uiNewEntry ();
  uiEntrySetReadOnly (entry, 1);
  uiButtonOnClicked (button, onOpenFileClicked, entry);
  uiGridAppend (grid, uiControl (button), 0, 0, 1, 1, 0, uiAlignFill, 0, uiAlignFill);
  uiGridAppend (grid, uiControl (entry), 1, 0, 1, 1, 1, uiAlignFill, 0, uiAlignFill);

  button = uiNewButton ("Open Folder");
  entry  = uiNewEntry ();
  uiEntrySetReadOnly (entry, 1);
  uiButtonOnClicked (button, onOpenFolderClicked, entry);
  uiGridAppend (grid, uiControl (button), 0, 1, 1, 1, 0, uiAlignFill, 0, uiAlignFill);
  uiGridAppend (grid, uiControl (entry), 1, 1, 1, 1, 1, uiAlignFill, 0, uiAlignFill);

  button = uiNewButton ("  Save File  ");
  entry  = uiNewEntry ();
  uiEntrySetReadOnly (entry, 1);
  uiButtonOnClicked (button, onSaveFileClicked, entry);
  uiGridAppend (grid, uiControl (button), 0, 2, 1, 1, 0, uiAlignFill, 0, uiAlignFill);
  uiGridAppend (grid, uiControl (entry), 1, 2, 1, 1, 1, uiAlignFill, 0, uiAlignFill);

  uiGrid *msggrid = uiNewGrid ();
  uiGridSetPadded (msggrid, 1);
  uiGridAppend (grid, uiControl (msggrid), 0, 3, 2, 1, 0, uiAlignCenter, 0, uiAlignStart);

  button = uiNewButton ("Message Box");
  uiButtonOnClicked (button, onMsgBoxClicked, NULL);
  uiGridAppend (msggrid, uiControl (button), 0, 0, 1, 1, 0, uiAlignFill, 0, uiAlignFill);
  button = uiNewButton ("Error Box");
  uiButtonOnClicked (button, onMsgBoxErrorClicked, NULL);
  uiGridAppend (msggrid, uiControl (button), 1, 0, 1, 1, 0, uiAlignFill, 0, uiAlignFill);
